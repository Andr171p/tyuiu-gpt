...
from src.core.entities import BaseMessage, DateToCount
...
class MessageRepository(BaseRepository):
    ...
    async def get_by_chat_id(self, chat_id: str) -> List[Optional[BaseMessage]]:
    ...
    async def count_per_day(self) -> List[Optional[DateToCount]]:
        count_per_day = await self._crud.read_count_per_day()
        return [
            DateToCount(date=date, count=count)
